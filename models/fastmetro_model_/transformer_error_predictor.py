...
class Transformer(nn.Module):
    ...
    def forward(self, img_features, jv_tokens, pos_embed, attention_mask=None):
        ...
        # Transformer Encoder 
        enc_img_features = self.encoder(img_features, src_key_padding_mask=mask, pos=pos_embed) # (1 + height * width) X batch_size X feature_dim

        # Transformer Decoder
        zero_tgt = torch.zeros_like(jv_tokens) # (num_joints + num_vertices) X batch_size X feature_dim
        jv_features, decoder_self_att_map = self.decoder(jv_tokens, enc_img_features, tgt_mask=attention_mask,
                                   memory_key_padding_mask=mask, pos=pos_embed, query_pos=zero_tgt) # (num_joints + num_vertices) X batch_size X feature_dim

        return enc_img_features, jv_features, decoder_self_att_map
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        output, att_map = output # output token, attention map
        if self.norm is not None:
            output = self.norm(output)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        #                     key_padding_mask=tgt_key_padding_mask)[0] # original code
        tgt2, att_map = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)
        att_map_list.append(att_map.detach().cpu().numpy())
