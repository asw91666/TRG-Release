...
class Transformer(nn.Module):
    ...
    def forward(self, img_features, bbox_token, cam_token, v_tokens, pos_embed, attention_mask=None):
        device = img_features.device
        hw, bs, _ = img_features.shape # (height * width), batch_size, feature_dim
        mem_mask = torch.zeros((bs, hw), dtype=torch.bool, device=device) # batch_size X (height * width)
        ...
        img_features = self.encoder(img_features, src_key_padding_mask=mem_mask, pos=pos_embed) # (1 + height * width) X batch_size X feature_dim
        img_bbox_features = torch.cat([img_features, bbox_token], dim=0) # (height * width + 1) X batch_size X feature_dim
        # cam_features, enc_img_features = e_outputs.split([1, hw], dim=0)

        # Transformer Decoder
        num_vtx = v_tokens.shape[0]
        bbox_mask = torch.zeros((bs, 1), dtype=torch.bool, device=device)  # batch_size X 1
        mem_mask_dec = torch.cat([mem_mask, bbox_mask], dim=1) # batch_size X (height * width + 1)
        vtx_cam_tokens = torch.cat([v_tokens, cam_token], dim=0) # [num_vertices+1] X batch_size X feature_dim
        zero_tgt = torch.zeros_like(vtx_cam_tokens) # (num_vertices+1) X batch_size X feature_dim
        d_output = self.decoder(vtx_cam_tokens, img_bbox_features, tgt_mask=attention_mask,
                                   memory_key_padding_mask=mem_mask_dec, pos=pos_embed, query_pos=zero_tgt) # (num_joints + num_vertices) X batch_size X feature_dim

        vtx_features, cam_features = d_output.split([num_vtx, 1], dim=0)
        return cam_features, img_features, vtx_features
...
class TransformerDecoderLayer(nn.Module):
    ...
    def with_pos_embed_memory(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else torch.cat([tensor[:-1] + pos, tensor[-1:]], dim=0)
    ...
    def forward(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed_memory(memory, pos),
